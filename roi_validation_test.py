...
def create_roi_test_bot():
    ...
    record_test(
        "ROI Test Bot Creation",
        True,
        f"ROI_Test_Bot_2 created with ID: {bot_id}, ROI 9% preset applied"
    )
...
def verify_roi_active_field(bot_id: str, admin_token: str):
    ...
    for bot in bots:
        if bot.get("id") == bot_id or bot.get("name") == "ROI_Test_Bot_2":
            roi_test_bot = bot
            break
    ...
    print(f"   📊 Found ROI_Test_Bot_2 in admin bots response")
