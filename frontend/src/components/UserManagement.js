...
const UserManagement = ({ user: currentUser }) => {
  ...
  const DeleteModal = () => {
    // Автофокус на поле ввода причины при открытии модального окна
    React.useEffect(() => {
      const textArea = document.querySelector('textarea[placeholder*="причину удаления"]');
      if (textArea) {
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-text-secondary text-sm font-rajdhani mb-1">
                Причина удаления *
                <span className="text-red-400 text-xs ml-1">(обязательно для заполнения)</span>
              </label>
              <textarea
                value={deleteReason}
                onChange={(e) => setDeleteReason(e.target.value)}
                placeholder="Подробно опишите причину удаления аккаунта..."
                className="w-full px-3 py-2 bg-surface-sidebar border border-border-primary rounded-lg text-white font-roboto focus:border-accent-primary focus:ring-1 focus:ring-accent-primary transition-all"
                rows="3"
                required
              ...
              {!deleteReason.trim() && (
                <p className="text-red-400 text-xs mt-1 opacity-75">
                  💡 Нажмите сюда и введите причину удаления для продолжения
                </p>
              )}
            </div>
