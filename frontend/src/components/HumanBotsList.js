...
const HumanBotsList = ({ 
  ...
  const [loadingToggleBetCreation, setLoadingToggleBetCreation] = useState(new Set());
  ...
  const handleTogglePlayWithPlayers = async (bot, canPlay) => {
    // Удалена проверка globalSettings.play_with_players_enabled - используются индивидуальные настройки
    // Переключение статуса "Играть с игроками"
            ...
            {humanBots.length === 0 ? (
              ...
              humanBots.map((bot, index) => (
                <tr key={bot.id} className="hover:bg-surface-sidebar hover:bg-opacity-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-center">
                    <label className={`relative inline-flex items-center group ${loadingToggleBetCreation.has(bot.id) ? 'cursor-wait' : 'cursor-pointer'}`}>
                      <input 
                        type="checkbox" 
                        checked={bot.is_bet_creation_active !== undefined ? bot.is_bet_creation_active : true}
                        onChange={(e) => handleToggleBetCreation(bot)}
                        disabled={loadingToggleBetCreation.has(bot.id)}
                        className="sr-only peer"
                      />
                      <div className={`relative w-11 h-6 bg-gray-700 rounded-full peer peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-green-300 peer-focus:ring-opacity-50 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300 peer-checked:bg-gradient-to-r peer-checked:from-green-500 peer-checked:to-emerald-600 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-105 ${loadingToggleBetCreation.has(bot.id) ? 'opacity-50' : ''}`}>
                        <div className="absolute inset-0 rounded-full bg-gradient-to-r from-transparent to-white opacity-20"></div>
                        {loadingToggleBetCreation.has(bot.id) && (
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-center">
                    <label className={`relative inline-flex items-center group ${loadingToggleAutoPlay.has(bot.id) ? 'cursor-wait' : 'cursor-pointer'}`}>
                      <input 
                        type="checkbox" 
                        checked={bot.can_play_with_other_bots || false}
                        onChange={(e) => handleToggleAutoPlay(bot, e.target.checked)}
                        disabled={loadingToggleAutoPlay.has(bot.id)}
                        className="sr-only peer"
                      />
                      <div className={`relative w-11 h-6 bg-gray-700 rounded-full peer peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 peer-focus:ring-opacity-50 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300 peer-checked:bg-gradient-to-r peer-checked:from-blue-500 peer-checked:to-purple-600 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-105 ${loadingToggleAutoPlay.has(bot.id) ? 'opacity-50' : ''}`}>
                        <div className="absolute inset-0 rounded-full bg-gradient-to-r from-transparent to-white opacity-20"></div>
                        {loadingToggleAutoPlay.has(bot.id) && (
                          <div className="absolute inset-0 flex items-center justify-center">
                            <svg className="animate-spin h-3 w-3 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
