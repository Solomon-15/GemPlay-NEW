...
async def get_human_bot_active_bets_count(bot_id: str) -> int:
    """Get count of active bets for a human bot."""
    try:
...
async def update_human_bots_settings(
    ...
    return {
        ...
        "old_max_limit": 100,
        "new_max_limit": settings.max_active_bets_human
    }
...
async def get_human_bot_active_bets(
    ...
    """Get active bets for a specific human bot."""
    try:
        # Find the bot
        bot = await db.human_bots.find_one({"id": bot_id})
        if not bot:
        ...
        # Find active games where this bot is the creator
        active_games = await db.games.find({
            "$and": [
                {"creator_id": bot_id},
                {"status": {"$in": ["WAITING", "REVEAL", "ACTIVE"]}},
        ...
        for game in active_games:
            # Get opponent info if exists
            opponent_name = "Ожидает игрока"
            if game.get("opponent_id"):
                if game.get("is_opponent_human_bot"):
                    opponent_bot = await db.human_bots.find_one({"id": game["opponent_id"]})
                    opponent_name = opponent_bot["name"] if opponent_bot else f"Human-bot {game['opponent_id'][:6]}"
                else:
                    opponent_user = await db.users.find_one({"id": game["opponent_id"]})
                    opponent_name = opponent_user["username"] if opponent_user else f"User {game['opponent_id'][:6]}"
            
            # Calculate time until cancel (15 minutes for human bot games)
            cancel_time = game["created_at"] + timedelta(minutes=15)
            time_left = cancel_time - datetime.utcnow()
            time_until_cancel = max(0, int(time_left.total_seconds()))
            ...
            bets.append({
                ...
                "time_until_cancel": time_until_cancel,
                "gems": game.get("creator_gems", {}),
                "total_gems": sum(game.get("creator_gems", {}).values()) if game.get("creator_gems") else 0
            })
...
async def get_human_bot_all_bets(
    ...
    """Get all bets history for a specific human bot."""
    try:
        # Find the bot
        bot = await db.human_bots.find_one({"id": bot_id})
        if not bot:
        ...
        # Find all games where this bot participated (as creator)
        all_games_cursor = db.games.find({
            "$and": [
                {"creator_id": bot_id},
                {"is_human_bot": True}
            ]
        ...
        games = await all_games_cursor.skip(skip).limit(limit).to_list(length=limit)
        
        # Format games data
        bets = []
        for game in games:
            # Get opponent info if exists
            opponent_name = "Не завершена"
            if game.get("opponent_id"):
                if game.get("is_opponent_human_bot"):
                    opponent_bot = await db.human_bots.find_one({"id": game["opponent_id"]})
                    opponent_name = opponent_bot["name"] if opponent_bot else f"Human-bot {game['opponent_id'][:6]}"
            ...
            # Determine result for this bot
            result = "В процессе"
            if game["status"] == "COMPLETED":
                if game.get("winner_id") == bot_id:
                    result = "Победа"
                elif game.get("winner_id") is None:
                    result = "Ничья"
                else:
                    result = "Поражение"
            ...
            bets.append({
                ...
                "result": result,
                "created_at": game["created_at"].isoformat(),
                "completed_at": game.get("completed_at").isoformat() if game.get("completed_at") else None,
                "opponent": opponent_name,
                "gems": game.get("creator_gems", {}),
                "total_gems": sum(game.get("creator_gems", {}).values()) if game.get("creator_gems") else 0,
                "winnings": game.get("winner_winnings", 0) if game.get("winner_id") == bot_id else 0
            })
