...
async def join_human_bot_bet(human_bot: HumanBot):
    ...
    try:
        ...
        commission_amount = round_money(selected_game.bet_amount * commission_rate)  # commission from winner only
...
async def create_game(
    ...
    try:
        ...
        # Check if user has enough balance for commission (configured bet_commission_rate)
        commission_rate = await get_bet_commission_rate_fraction()
...
async def determine_game_winner(game_id: str) -> dict:
    ...
    try:
        ...
        if winner_id and not is_regular_bot_game:
            commission_rate = await get_bet_commission_rate_fraction()
            commission_amount = round_money(game_obj.bet_amount * commission_rate)  # winner pays commission
            ...
            if winner:
                ...
                if not is_regular_bot_game and commission_amount > 0:
                    ...
                    profit_entry = ProfitEntry(
                        ...
                        reference_id=game.id,
                        description=f"{int((await get_bet_commission_rate_fraction())*100)}% commission from PvP game winner (${game.bet_amount} bet)"
                    )
                    await db.profit_entries.insert_one(profit_entry.dict())
        ...
        for game in incomplete_games:
            ...
            if opponent_id:
                ...
                if opponent and opponent.get("frozen_balance", 0) >= commission_per_player:
                    ...
                    game_info["players_processed"].append({
                        "user_id": opponent_id,
                        "username": opponent.get("username", "Unknown Player"),
                        "amount_unfrozen": commission_per_player
                    })
